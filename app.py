...
@app.route('/ai/insights/<child_id>')
def ai_insights(child_id):
    """Return JSON insights generated by the AI engine for a child.

    Access is limited to logged-in users; the engine will load models and fall back
    ...
    if account_type == 'parent':
        ...
        if not any(c.get('id') == child_id for c in children):
